...
import { Plus, Trash2, Building2, ListChecks, UserCircle } from 'lucide-react'
...
import type { DashboardUser, ChecklistItem, Unit } from '@/lib/types'
...
export function Configuracoes({
  ...
  function addUnidade() {
    if (!nomeUnidade.trim()) {
      toast.error('Informe o nome/número da unidade.')
      return
    }
    startTransition(async () => {
      try {
        await createUnidade({
          name: nomeUnidade.trim(),
          itemIds: itens.map((i) => i.legacyId),
        })
    ...
    <div className="space-y-6">
      <Tabs defaultValue="unidades" className="w-full">
        ...
        <TabsContent value="unidades" className="mt-6 space-y-6">
          <Panel title="Adicionar unidade">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
              <div className="flex flex-col gap-2 sm:col-span-3">
                <Label htmlFor="nomeUnidade">Nome / número</Label>
                <Input
                  id="nomeUnidade"
                  value={nomeUnidade}
                  onChange={(e) => setNomeUnidade(e.target.value)}
                  placeholder="Ex.: 101 ou Studio 101"
          ...
          <Panel
            ...
            description={`${unidades.length} no total`}
          >
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {unidades.map((u) => (
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent text-sm font-semibold text-accent-foreground">
                    {u.name.slice(0, 2)}
                  </div>
