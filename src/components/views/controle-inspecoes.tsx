...
import { createInspecao, deleteInspecao } from '@/app/actions/data'
import type {
  InspecaoComUnidade,
  ChecklistItem,
  Unit,
...
export function ControleInspecoes({
  ...
  const [statusItens, setStatusItens] = useState<Record<number, boolean>>({})
  ...
  function handleSubmit() {
    ...
    const itensPayload = itens.map((it) => ({
      itemLegacyId: it.legacyId,
      status: (statusItens[it.legacyId] === false
        ? 'NAO_CONFORME'
        : 'CONFORME') as 'CONFORME' | 'NAO_CONFORME',
      comment: obsItens[it.legacyId] || undefined,
    }))
  ...
  const problemasSelecionados = itens.filter(
    (it) => statusItens[it.legacyId] === false,
  ).length
              ...
              <div className="max-h-[60svh] space-y-5 overflow-y-auto px-6 py-5">
                ...
                <div>
                  ...
                  <div className="space-y-2 rounded-2xl border border-border/70 p-2">
                    {itens.map((it) => {
                      const problema = statusItens[it.legacyId] === false
                      return (
                        <div
                          key={it.id}
                        ...
                        >
                          ...
                          {problema && (
                            <Input
                              value={obsItens[it.legacyId] ?? ''}
                              onChange={(e) =>
                                setObsItens((s) => ({
                                  ...s,
                                  [it.legacyId]: e.target.value,
                                }))
                              }
